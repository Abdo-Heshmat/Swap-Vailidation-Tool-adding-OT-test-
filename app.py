...
is_ramadan = st.checkbox("🌙 Ramadan Mode (7h Shifts)", key="ramadan_mode", on_change=lambda: [update_end_time(i, wk) for i in [1, 2] for wk in ["Current", "Next"]])
    ...
    with col:
        ...
        for wk in ["Current", "Next"]:
            with st.container(border=True):
                ...
                with t_cols[0]:
                    st.selectbox(f"S{i}{wk}", hrs, key=f"s{i}{wk}", on_change=update_end_time, args=(i, wk), label_visibility="collapsed")
                t_cols[1].markdown("<p style='text-align:center; padding-top:10px;'>to</p>", unsafe_allow_html=True)
                with t_cols[2]:
                    st.selectbox(f"E{i}{wk}", hrs, key=f"e{i}{wk}", label_visibility="collapsed")
                ...
                with st.expander("➕ Daily OT (Max 2h)"):
                    b_v = st.session_state.get(f"otb_{i}_{wk}", 0)
                    a_v = st.session_state.get(f"ota_{i}_{wk}", 0)
                    st.number_input(f"B{i}{wk}", 0, 2 - a_v, key=f"otb_{i}_{wk}", label_visibility="collapsed")
                    st.number_input(f"A{i}{wk}", 0, 2 - b_v, key=f"ota_{i}_{wk}", label_visibility="collapsed")

            r_offs = []
            if o1 in days and not st.session_state.get(f"do_ot1_{i}_{wk}", False): r_offs.append(days.index(o1)+1)
            if o2 in days and not st.session_state.get(f"do_ot2_{i}_{wk}", False): r_offs.append(days.index(o2)+1)
            shift_data[f"e{i}_{wk}"] = {"s": st.session_state[f"s{i}{wk}"], "e": st.session_state[f"e{i}{wk}"], "off": sorted(r_offs)}
...
if st.button("🚀 Run Swap Check", use_container_width=True):
    if not all_days_selected:
        ...
        for s in swaps:
            ...
            results.append({"name": s["name"], "r_pass": gap >= 12 or is_exempt, "r_msg": "Waived (Off Saturday or Sunday)." if is_exempt else f"{gap:.1f}h gap (Min 12h).", "s_pass": streak <= 6, "s_val": f"{streak} consecutive days (Max 6)."})

        all_pass = all(r["r_pass"] and r["s_pass"] for r in results)
        st.markdown(f"<div class='results-card'>", unsafe_allow_html=True)
        st.markdown(f"## {'✅ Swap Approved' if all_pass else '❌ Swap Rejected'}", unsafe_allow_html=True)
        for r in results:
...
st.markdown(f'<div class="footer">Created by Abdelrahman Heshmat - chime me @abheshma</div>', unsafe_allow_html=True)
