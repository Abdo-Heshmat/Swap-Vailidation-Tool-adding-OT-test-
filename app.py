...
def on_load_random():
    names = ["Abdelrahman", "Sarah", "Ahmed", "Mariam"]
    for i in [1, 2]:
        st.session_state[f"un{i}"] = random.choice(names)
    ...
    with col:
        ...
        for wk in ["Current", "Next"]:
            with st.container(border=True):
                ...
                off1 = st.selectbox(f"O1{i}{wk}", ["1st Day Off"] + days, key=f"o1{i}{wk}", label_visibility="collapsed")
                do_ot1 = st.toggle("Work OT (Day 1)", key=f"do_ot1_{i}_{wk}")
                off2 = st.selectbox(f"O2{i}{wk}", ["2nd Day Off"] + [d for d in days if d != off1], key=f"o2{i}{wk}", label_visibility="collapsed")
                do_ot2 = st.toggle("Work OT (Day 2)", key=f"do_ot2_{i}_{wk}")
                
                with st.expander("➕ OT (Max 2h Total)"):
                    # Dynamic OT logic: total must not exceed 2
                    cur_b = st.session_state.get(f"otb_{i}_{wk}", 0)
                    cur_a = st.session_state.get(f"ota_{i}_{wk}", 0)
                    st.number_input("Before", 0, 2 - cur_a, key=f"otb_{i}_{wk}")
                    st.number_input("After", 0, 2 - cur_b, key=f"ota_{i}_{wk}")

            offs = []
            if off1 in days and not do_ot1: offs.append(days.index(off1) + 1)
            if off2 in days and not do_ot2: offs.append(days.index(off2) + 1)
            shift_data[f"e{i}_{wk}"] = {"s": s_t, "e": e_t, "off": offs}
...
if st.button("🚀 Run Swap Check", use_container_width=True):
    ...
    for en, cfg in configs.items():
        ...
        if is_exempt:
            reasons = ["✅ **Rest Rule:** Waived (Off Saturday or Sunday)."]
        else:
            ...
            reasons = [f"{'✅' if gap >= 12 else '❌'} **Rest Rule:** {gap:.1f}h gap (Min 12h)."]
