...
# Custom CSS for Centering and UI
st.markdown(f"""
    <style>
...
for i, col in enumerate([c1, c2], 1):
    with col:
        ...
        st.text_input(f"Name {i}", key=f"un{i}", placeholder=f"Employee Name", label_visibility="collapsed")
...
if st.button("🚀 Run Swap Check", use_container_width=True):
    ...
    st.markdown(f"<div style='background-color:{'#1b5e20' if success else '#b71c1c'}; padding:20px; border-radius:12px;'>", unsafe_allow_html=True)
    ...
    for r in results:
        st.markdown(f"<b style='color:white;'>{r['name']}</b>", unsafe_allow_html=True)
