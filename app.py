...
def get_dt(day_idx, time_str, is_end=False, s_time_str=None):
    base = datetime(2026, 3, 22) # Start of week
...
with h_col: 
    st.markdown(f'<div class="title-container"><h1>👤🔁👤 Smart Swap Validator Pro</h1></div>', unsafe_allow_html=True)
...
for i, col in enumerate([c1, c2], 1):
    with col:
        ...
        st.text_input(f"Name {i}", key=f"un{i}", placeholder="Enter Employee Name", label_visibility="collapsed")
        ...
        for wk in ["Current", "Next"]:
            with st.container(border=True):
                ...
                # Day Off & Mutual Exclusion Toggles
                ...
                # Daily OT (Max 2h combined)
                with st.expander("➕ Daily OT (Max 2h)"):
                    cb, ca = st.session_state.get(f"otb_{i}_{wk}", 0), st.session_state.get(f"ota_{i}_{wk}", 0)
                    st.number_input("OT Before", 0, 2 - ca, key=f"otb_{i}_{wk}")
                    st.number_input("OT After", 0, 2 - cb, key=f"ota_{i}_{wk}")

            # Store only verified days off
            real_offs = []
...
if st.button("🚀 Run Swap Check", use_container_width=True):
    ...
    for en, cfg in configs.items():
        ...
        first_off_next = shift_data[cfg['n']]["off"][0] if
